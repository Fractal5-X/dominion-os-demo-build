...
class CodeGenerator(NodeVisitor):
    ...
    def pull_dependencies(self, nodes: t.Iterable[nodes.Node]) -> None:
        ...
        for id_map, names, dependency in (
            ...
            for name in sorted(names):
                ...
                self.indent()
                self.writeline(
                    f'raise TemplateRuntimeError("No {dependency[:-1]}' f' named {name!r} found.")'
                )
                self.outdent()
                self.outdent()
    ...
    def dump_local_context(self, frame: Frame) -> str:
        items_kv = ", ".join(
            f"{name!r}: {target}" for name, target in frame.symbols.dump_stores().items()
        )
        return f"{{{items_kv}}}"
    ...
    def visit_FromImport(self, node: nodes.FromImport, frame: Frame) -> None:
        ...
        for name in node.names:
            if isinstance(name, tuple):
                ...
                alias = name
            self.writeline(
                f"{frame.symbols.ref(alias)} =" f" getattr(included_template, {name!r}, missing)"
            )
